defmodule EctoOne.RepoTest do
  ...
  test "needs model with primary key field" do
    ...
    assert_raise EctoOne.NoPrimaryKeyFieldError, fn ->
      TestRepo.update!(model |> EctoOne.Changeset.change, force: true)
    end
  ...
  test "validates model types" do
    ...
    assert_raise EctoOne.ChangeError, fn ->
  ...
  test "validates get" do
    ...
    message = "cannot perform EctoOne.TestRepo.get/2 because the given value is nil"
    ...
    assert_raise EctoOne.QueryError, message, fn ->
      TestRepo.get(%EctoOne.Query{}, :atom)
    end
  end
  ...
  test "insert, update, insert_or_update and delete accepts changesets" do
    valid = EctoOne.Changeset.cast(%MyModel{id: 1}, %{}, [], [])
  ...
  test "insert, update, insert_or_update and delete errors on invalid changeset" do
    invalid = %EctoOne.Changeset{valid?: false, model: %MyModel{}}
  ...
  test "insert!, update!, insert_or_update! and delete! fail on invalid changeset" do
    ...
    assert_raise EctoOne.InvalidChangesetError,
  ...
  test "insert!, update! and delete! fail on changeset without model" do
    invalid = %EctoOne.Changeset{valid?: true, model: nil}
  ...
  test "insert!, update!, insert_or_update! and delete! fail on changeset with wrong action" do
    invalid = %EctoOne.Changeset{valid?: true, model: %MyModel{}, action: :other}

    assert_raise ArgumentError, "a changeset with action :other was given to EctoOne.TestRepo.insert/2", fn ->
      TestRepo.insert!(invalid)
    end

    assert_raise ArgumentError, "a changeset with action :other was given to EctoOne.TestRepo.update/2", fn ->
      TestRepo.update!(invalid)
    end
    ...
    assert_raise ArgumentError, "a changeset with action :other was given to EctoOne.TestRepo.delete/2", fn ->
      TestRepo.delete!(invalid)
    end
  end

  test "insert_or_update uses the correct action" do
    built  = EctoOne.Changeset.cast(%MyModel{y: "built"}, %{}, [], [])
    loaded =
      %MyModel{y: "loaded"}
      |> TestRepo.insert!
      |> EctoOne.Changeset.cast(%{y: "updated"}, [:y], [])
  ...
  defp prepare_changeset() do
    %MyModel{id: 1}
    |> EctoOne.Changeset.cast(%{x: "one"}, [:x], [])
    |> EctoOne.Changeset.prepare_changes(fn %{repo: repo} = changeset ->
          Process.put(:ecto_one_repo, repo)
          Process.put(:ecto_one_counter, 1)
          changeset
        end)
    |> EctoOne.Changeset.prepare_changes(fn changeset ->
          Process.put(:ecto_one_counter, 2)
          changeset
        end)
  end
  ...
  test "insert runs prepare callbacks in transaction" do
    ...
    assert Process.get(:ecto_one_repo) == TestRepo
    assert Process.get(:ecto_one_counter) == 2
  end
