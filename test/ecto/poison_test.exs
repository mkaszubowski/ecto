defmodule EctoOne.PoisonTest do
  use ExUnit.Case, async: true

  test "encodes datetimes" do
    time = %EctoOne.Time{hour: 1, min: 2, sec: 3}
    assert Poison.encode!(time) == ~s("01:02:03")

    date = %EctoOne.Date{year: 2010, month: 4, day: 17}
    assert Poison.encode!(date) == ~s("2010-04-17")

    dt = %EctoOne.DateTime{year: 2010, month: 4, day: 17, hour: 0, min: 0, sec: 0}
