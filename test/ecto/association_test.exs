defmodule EctoOne.AssociationTest do
  use ExUnit.Case, async: true
  doctest EctoOne.Association
  ...
  test "has many" do
    ...
    assert inspect(EctoOne.Association.Has.joins_query(assoc)) ==
           inspect(from p in Post, join: c in Comment, on: c.post_id == p.id)

    assert inspect(EctoOne.Association.Has.assoc_query(assoc, [])) ==
           inspect(from c in Comment, where: c.post_id in ^[])

    assert inspect(EctoOne.Association.Has.assoc_query(assoc, [1, 2, 3])) ==
  ...
  test "has many custom assoc query" do
    ...
    assert inspect(EctoOne.Association.Has.assoc_query(assoc, query, [1, 2, 3])) ==
  ...
  test "has many through many to many" do
    ...
    assert inspect(EctoOne.Association.HasThrough.joins_query(assoc)) ==
           inspect(from a in Author, join: p in assoc(a, :posts), join: c in assoc(p, :comments))

    assert inspect(EctoOne.Association.HasThrough.assoc_query(assoc, [1,2,3])) ==
  ...
  test "has many through custom assoc many to many query" do
    ...
    assert inspect(EctoOne.Association.HasThrough.assoc_query(assoc, query, [1,2,3])) ==
  ...
  ## Integration tests through EctoOne
  ...
  test "sets association to loaded/not loaded" do
    refute EctoOne.assoc_loaded?(%Post{}.comments)
    assert EctoOne.assoc_loaded?(%Post{comments: []}.comments)
  end
  ...
  test "preload: expand" do
    assert [{:comments, {:assoc, %EctoOne.Association.Has{}, :post_id}, {nil, []}},
            {:permalink, {:assoc, %EctoOne.Association.Has{}, :post_id}, {nil, []}}] =
           expand(Post, [:comments, :permalink])

    assert [{:post, {:assoc, %EctoOne.Association.BelongsTo{}, :id},
    ...
    assert [{:posts, {:assoc, %EctoOne.Association.Has{}, :author_id}, {nil, [comments: {nil, [post: {nil, []}]}]}},
            {:posts_comments, {:through, %EctoOne.Association.HasThrough{}, [:posts, :comments]}, {nil, []}}] =
           expand(Author, [posts_comments: :post])

    assert [{:posts, {:assoc, %EctoOne.Association.Has{}, :author_id}, {nil, [comments: _, comments: _]}},
           {:posts_comments, {:through, %EctoOne.Association.HasThrough{}, [:posts, :comments]}, {nil, []}}] =
           expand(Author, [:posts, posts_comments: :post, posts: [comments: :post]])

    query = from(c in Comment, limit: 1)
    assert [{:permalink, {:assoc, %EctoOne.Association.Has{}, :post_id}, {nil, []}},
            {:comments, {:assoc, %EctoOne.Association.Has{}, :post_id}, {^query, []}}] =
           expand(Post, [:permalink, comments: query])

    assert [{:posts, {:assoc, %EctoOne.Association.Has{}, :author_id}, {nil, [comments: {^query, [post: {nil, []}]}]}},
            {:posts_comments, {:through, %EctoOne.Association.HasThrough{}, [:posts, :comments]}, {nil, []}}] =
