defmodule EctoOne do
  @moduledoc ~S"""
  EctoOne is split into 4 main components:

    * `EctoOne.Repo` - repositories are wrappers around the database.
      Via the repository, we can create, update, destroy and query existing entries.
      A repository needs an adapter and a URL to communicate to the database

    * `EctoOne.Schema` - schemas allow developers to define data structures
      that map to the underlying storage

    * `EctoOne.Changeset` - changesets provide a way for developers to filter
      and cast external parameters, as well as a mechanism to track and
      validate changes before their are sent to the database

    * `EctoOne.Query` - written in Elixir syntax, queries are used to retrieve
      information from a given repository. Queries in EctoOne are secure, avoiding
      common problems like SQL Injection, and also provide type safety. Queries
      are composable via the `EctoOne.Queryable` protocol

  In the following sections, we will provide an overview of those components and
  how they interact with each other. Feel free to access their respective module
  documentation for more specific examples, options and configuration.

  If you want to quickly check a sample application using EctoOne, please check
  https://github.com/elixir-lang/ecto_one/tree/master/examples/simple.

  ## Repositories

  `EctoOne.Repo` is a wrapper around the database. We can define a
  ...
  Each repository in EctoOne defines a `start_link/0` function that needs to be invoked
  ...
  > NOTE: by using `EctoOne.Schema`, an `:id` field with type `:integer` is
  ...
  > before the `schema/2` call. Consult the `EctoOne.Schema` documentation
  ...
  so EctoOne can efficiently track changes.
  ...
  The `changeset/2` function first invokes `EctoOne.Changeset.cast/4` with
  ...
  After casting, the changeset is given to many `EctoOne.Changeset.validate_*/2`
  ...
  proper feedback to end users. Check `EctoOne.Changeset.unique_constraint/3`
  for some examples as well as the other `_constraint` functions.

  ## Query

  Last but not least, EctoOne allows you to write queries in Elixir and send
  ...
  in the `EctoOne.Query` module. Functions supported in queries are listed
  in `EctoOne.Query.API`.
  ...
  When an association is defined, EctoOne also defines a field in the schema
  ...
  The `EctoOne.Model` module also provides conveniences for working
  with associations. For example, `EctoOne.assoc/2` returns a query
  ...
  Another function in `EctoOne` is `build_assoc/3`, which allows
  someone to build an associated struct with the proper fields:
      ...
      Repo.transaction fn ->
        ...
        comment = EctoOne.build_assoc(post, :comments, body: "Excellent!")

        Repo.insert!(comment)
      end

  In the example above, `EctoOne.build_assoc/3` is equivalent to:

      %Comment{post_id: post.id, body: "Excellent!"}

  You can find more information about defining associations and each
  respective association module in `EctoOne.Schema` docs.

  > NOTE: EctoOne does not lazy load associations. While lazily loading
  > associations may sound convenient at first, in the long run it
  > becomes a source of confusion and performance issues.

  ### Embeds

  EctoOne also supports embeds. While associations keep parent and child
  ...
  Check `EctoOne.Schema.embeds_one/3` and `EctoOne.Schema.embeds_many/3`
  for more information.

  ### Mix tasks and generators

  EctoOne provides many tasks to help your workflow as well as code generators.
  You can find all available tasks by typing `mix help` inside a project
  with EctoOne listed as a dependency.

  EctoOne generators will automatically open the generated files if you have
  `ECTO_EDITOR` set in your environment variable.

  #### Migrations

  EctoOne supports database migrations. You can generate a migration with:
  ...
  Our of the box, EctoOne tasks assumes that the location of your Repo lives within
  your application's root namespace; for example, in the previous examples, EctoOne
  will assume your Repo is located at MyApp.Repo.

  For more complex use-cases, this might not be sufficient and EctoOne allows you
  ...
  """
  @spec primary_key(EctoOne.Schema.t) :: Keyword.t
  ...
  Raises `EctoOne.NoPrimaryKeyFieldError` if the model has no
  primary key field.
  """
  @spec primary_key!(EctoOne.Schema.t) :: Keyword.t | no_return
  def primary_key!(struct) do
    case primary_key(struct) do
      [] -> raise EctoOne.NoPrimaryKeyFieldError, model: struct
      pk -> pk
    end
  end
  ...
  def build_assoc(%{__struct__: model} = struct, assoc, attributes \\ %{}) do
    assoc = EctoOne.Association.association_from_model!(model, assoc)
  ...
  def assoc(model_or_models, assoc) do
    ...
    assoc = %{owner_key: owner_key} =
      EctoOne.Association.association_from_model!(model, assoc)
  ...
  ## Examples
      ...
      iex> EctoOne.assoc_loaded?(post.comments)
      false
  ...
  """
  @spec put_meta(EctoOne.Schema.t, [source: String.t, prefix: String.t,
                                context: term, state: :built | :loaded | :deleted]) :: EctoOne.Schema.t
